...
const ChainUpdater: React.FC = () => {
    ...
    const {setActiveAuction, setFullAuction, appendBid, setAuctionSettled, setAuctionExtended} = useAuctionState()
    const {setOnDisplayAuctionNounId, setOnDisplayAuctionStartTime, setLastAuctionNounId, setLastAuctionStartTime} = useDisplayAuction()

    const loadState = async () => {
        const provider = new WebSocketProvider(config.app.wsRpcUri);
        const {nounletAuction, nounletToken} = CHAIN_ID === 4 ? getRinkebySdk(provider) : getMainnetSdk(provider) as RinkebySdk
        ...
        const currentAuction = await nounletAuction.auctionInfo('0x0000000000000000000000000000000000000000');
        ...
        nounletAuction.on(extendedFilter, (nounId, endTime) =>
